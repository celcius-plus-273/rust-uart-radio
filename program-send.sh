...
file_name="send.hex" 	# edit this to changee output file name
...
cargo build --release --example send

# compiles/builds the project and converts the file into an output hex file
rust-objcopy -O ihex target/thumbv7em-none-eabihf/release/examples/send $file_name
